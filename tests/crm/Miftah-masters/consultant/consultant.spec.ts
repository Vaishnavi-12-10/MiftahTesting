...
import serviceData from '../../../test-data/consultant/consultant.json';
...
test('Create Makeup Artist Service - Miftah Master Portal', async ({ page }) => {
    ...
    await categoryModal
        .getByRole('button', { name: /consultant/i })
        .click({ force: true });
    ...
    // ================= PRICING =================
    // Pricing fields are spinbuttons. Target the parent row of the label text.
    await page.getByText(/^base price \*/i).locator('xpath=ancestor::div[1]').getByRole('spinbutton').fill(serviceData.basePrice);
    ...
    // Industry Focus
    const industryFocus = page.getByPlaceholder(/enter industries/i);
    await industryFocus.scrollIntoViewIfNeeded();
    await expect(industryFocus).toBeVisible();
    await industryFocus.fill(serviceData.industryFocus);

    // Deliverables
    const deliverables = page.getByPlaceholder(/enter deliverables/i);
    await expect(deliverables).toBeVisible();
    await deliverables.fill(serviceData.deliverables);

    // Institution Focus
    const institutionFocus = page
        .getByText(/institution focus/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('textbox');

    await expect(institutionFocus).toBeVisible();
    await institutionFocus.fill(serviceData.institutionFocus);

    // Video Call Included? → Yes
    await page.getByRole('radio', { name: serviceData.videocallIncluded ? 'Yes' : 'No' });
    ...
    saveLogs(logs, 'create-consultant-service');

    console.log('Consultant service created and logged successfully.');
