...
import serviceData from '../../test-data/gardener.json';
...
test('Create Makeup Artist Service - Miftah Master Portal', async ({ page }) => {
    ...
    await categoryModal
        .getByRole('button', { name: /Gardener/i })
        .click({ force: true });
    ...
    const serviceSpecificsSection = page.getByRole('heading', { name: /service specifics/i });
    await serviceSpecificsSection.scrollIntoViewIfNeeded();

    // ===== Service Tasks =====
    await page
        .getByPlaceholder(/enter tasks/i)
        .fill(serviceData.serviceTasks.join(', '));

    // ===== Tools Included? → Yes =====
    await page
        .getByText(/tools included/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('radio', { name: serviceData.toolsIncluded ? 'Yes' : 'No' })
        .check();

    // ===== Waste Removal Included? → Yes =====
    await page
        .getByText(/waste removal included/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('radio', { name: serviceData.wasteRemovalincluded ? 'Yes' : 'No' })
        .check();

    // ===== Organic Methods? → No =====
    await page
        .getByText(/organic methods/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('radio', { name: serviceData.organicMethods ? 'Yes' : 'No' })
        .check();
    ...
    saveLogs(logs, 'create-gardener-service');

    console.log('Gardener service created and logged successfully.');
