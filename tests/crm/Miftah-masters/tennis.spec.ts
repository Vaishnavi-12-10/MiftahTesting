...
import serviceData from '../../test-data/tennis.json';
...
test('Create Makeup Artist Service - Miftah Master Portal', async ({ page }) => {
    ...
    // ===== Court Surface Specialty =====
    await page
        .getByPlaceholder(/enter specialties/i)
        .fill(serviceData.courtsurfacespecialty);

    // ===== ITF Certification Level =====
    await page
        .getByText(/itf certification level/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('textbox')
        .fill(serviceData.itfcertificationlevel);

    // ===== Session Duration (Mins) =====
    await page
        .getByText(/session duration/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('spinbutton')
        .fill(serviceData.sessionduration);

    // ===== Court Included? → Yes =====
    await page
        .getByText(/court included/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('radio', { name: serviceData.courtincluded })
        .check();

    // ===== Max Students Per Session =====
    await page
        .getByText(/max students per session/i)
        .locator('xpath=ancestor::div[1]')
        .getByRole('spinbutton')
        .fill(serviceData.maxstudentspersession);
    ...
    saveLogs(logs, 'create-tennis-service');
